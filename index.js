...
app.post('/', async (req, res) => {
    ...
    //         },
    //         probability: {
    //             description: "the possibility(from the scale of 0.0 to 10.0) of the guess",
    //             type: "number"       
    //         },
    //         classification: {
    ...
    const {data, response } = await openai.chat.completions.create({
        ...
        // max_tokens: 2000,
        messages: [
            // Return one profession
            // {role: 'user', content: `Given the job description in []: [${message}], guess the full name of the most matched profession and provide more information about it , including at least the english name, french name, and classification of the matching profession and the probability that it is a match.`},
            // Return two profession
    ...
    console.log('profession_data', profession_data)
    // res.json({
    //     profession_name: profession_data.profession_name,
    //     probability: profession_data.probability ? profession_data.probability : 'Not Available',
    //     classification: profession_data.classification
    // });
    res.json({
        profession_1: {
